...
更新于 2026-09-04。App 门是 GT3 bind。本机 Node 黑盒 `stamp_gt3()`（不依赖 RuyiDOM）已把新游客 `get_cpt_ifm` 打成 100000。纯算 `w` 仍 `error_03`，不能标 algorithmic。网页章节链是另一条产品面。
...
| L3（运行时） | Pixel 6 原包过 Splash；panda 43 DEX；官方正文 MITM；stamp-event；GT3 一键；自注册官方打戳；新游客 oracle 复验；`client/gt3.py` API1；Node 黑盒 bind `100000` | dump 后进程被 panda 暂停致死 | 纯算 `w` 仍 `error_03` | fullpage 9.2.0 packing |
...
- **严重程度**：`resolved-node`（纯算 `w` 仍 pending）
- **原因**：门在身份有没有走过 GT3 bind。本机 Node 跑官方 `gt.js` bind 已出三元组，独立会话 `100000`，不依赖 RuyiDOM。AES+RSA packing 是 `error_03`。假 `|jordan` 是 280002。网页章节链风控不同。
- **已尝试的手段**：对照/新游客/JA3/身份移植/读章序/uid MITM/字段对照/官方独有接口/stamp-event/API1+假三元组/官方 prefs 注入 oracle/第二份新游客无 MITM 复验/Python bind 面骨架/RuyiDOM 对照/Node 黑盒/AES+RSA ajax。
- **证据**：`client/gt3_w.py`、`client/gt3_node_bind.mjs`、`evidence/gt3-node-bind-canary.json`、`client/gt3.py`、`test_gt3.py`
- **建议下一步**：采集调用 `stamp_gt3()`。要对齐纯算再冻 9.2.0 `w` fixture。

**当前处理**：App 门走本机 Node 黑盒。`client.web` 仍是另一条产品面。
